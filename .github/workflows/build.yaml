    ...
    steps:
      ...
      - name: Build depends (Android)
        ...
        run: |
          ...
          JOBS=$(getconf _NPROCESSORS_ONLN 2>/dev/null || nproc 2>/dev/null || sysctl -n hw.ncpu 2>/dev/null || echo 2)
          if grep -qE '^doctor:' depends/Makefile 2>/dev/null; then make -C depends doctor HOST=${HOST}; fi
          make -C depends -j"${JOBS}" HOST=${HOST} ANDROID_SDK="${ANDROID_SDK}" ANDROID_NDK="${ANDROID_NDK}" NO_QT=1 V=1 AR=llvm-ar RANLIB=llvm-ranlib STRIP=llvm-strip 2>&1 | tee -a depends.build.log
