    ...
    static const std::map<std::string, RPCArg> mapParamHelp = {
        {"collateralAddress",
            {"collateralAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The springbok address to send the collateral to."}
        },
        {"collateralHash",
        ...
        {"ownerAddress",
            {"ownerAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The springbok address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress_register",
            {"payoutAddress_register", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The springbok address to use for masternode reward payments."}
        },
...
static void protx_register_fund_help(const JSONRPCRequest& request)
{
    RPCHelpMan{"protx register_fund",
        "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 10000000 SPRINGBOK\n"
...
static UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (!request.params[paramIdx + 6].isNull()) {
        ...
        if (!IsValidDestination(fundDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid SPRINGBOK address: ") + request.params[paramIdx + 6].get_str());
    }
...
static UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (!request.params[5].isNull()) {
        ...
        if (!IsValidDestination(feeSource))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid SPRINGBOK address: ") + request.params[5].get_str());
    } else {
...
static UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (!request.params[4].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid SPRINGBOK address: ") + request.params[4].get_str());
        FundSpecialTx(pwallet, tx, ptx, feeSourceDest);
