...
static const char* qt_argv = "springbok-qt";
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // springbok: URIs or payment requests:
        if (paymentServer) {
...
{
    QMessageBox::critical(nullptr, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. SPRINGBOK Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"SPRINGBOK Core - \"").translated, ArgsManager::ALLOW_ANY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    /// 6. Determine availability of data and blocks directory and parse springbok.conf
    ...
    // springbok: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
