...
$(package)_patches+= fix_mingw_cross_compile.patch fix_qpainter_non_determinism.patch fix_limits_header.patch qt_unary_function_fix.patch qt_printsupport_skip.patch
...
$(package)_config_opts += -confirm-license -hostprefix $(build_prefix)
$(package)_config_opts += -no-compile-examples -no-cups -no-egl -no-eglfs -no-gif -no-glib
$(package)_config_opts += -no-ico -no-iconv -no-kms -no-linuxfb -no-libudev -no-mtdev -no-openvg
...
for patchfile in $($(package)_patches); do \
  patch -p1 -i $($(package)_patch_dir)/$$patchfile || exit 1; \
done
...
$(MAKE) -C qttranslations INSTALL_ROOT=$($(package)_staging_dir) install_target
endef

define $(package)_postprocess_cmds
echo "Qt $($(package)_version) build completed successfully."
endef
